...
DB = {}
require 'json'
require './store/lib/store.rb'

$store = Store.new('hanley-cmms')

$store.clear :equipment
$store.clear :tasks
...
Machine.iter.every(15).each do |a|
  ids.push(*$store.post(:equipment, (a.map do |m| m.to_h end)))
end

i=-1
Machine.iter.every(15).each do |a|
  $store.post :tasks, (a.map do |m| 
    i+=1
    m.tasks.each do |t| t["equipment"] = ids[i] end
    m.tasks 
  end.flatten)
end
