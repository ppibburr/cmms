...
def http method, *o, data: {}
  JSON.parse(`curl -X #{method.to_s.upcase} -d "#{data.to_json.inspect}" http://localhost:4567/api/#{o.join("/")} 2>/dev/null`)
end

def create_workorder priority: nil, type: nil, dept: nil, equip: nil, tasks: []
  wo = {
    priority: priority,
    type:     type,
    dept:     dept,
